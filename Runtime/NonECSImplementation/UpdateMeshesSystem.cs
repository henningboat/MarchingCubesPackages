...
namespace NonECSImplementation
{
    ...
    {
        private NativeList<CTriangulationInstruction> _triangulationInstructions;
        ...
        public void Initialize(GeometryFieldData geometryFieldData)
        {
            ...
            _triangulationInstructions = new NativeList<CTriangulationInstruction>(geometryFieldData.TotalSubChunkCount, Allocator.Persistent);
        ...
        public void Update()
        {
            _triangulationInstructions.Clear();
            JCalculateTriangulationIndicesJob job = new JCalculateTriangulationIndicesJob()
            {
                ...
                TriangulationInstructions = _triangulationInstructions.AsParallelWriter(),
        ...
        {
            [WriteOnly] public NativeList<CTriangulationInstruction>.ParallelWriter TriangulationInstructions;
            ...
            // ref CClusterParameters clusterParameters,
            ...
            public void Execute(int clusterIndex)
            {
                ...
                //     var vertexCountReadbackTimesStamp = 0;
                    ...
                    for (var chunkIndex = 0; chunkIndex < GeometryFieldData.chunksPerCluster; chunkIndex++)
                    {
                        ...
                        var positionOfChunkWS = Utils.IndexToPositionWS(chunkIndex, new int3(GeometryFieldData.chunkLength));
                        ...
                        for (var i = 0; i < 8; i++)
                        {
                            ...
                            {
                                var subChunkOffset = Utils.IndexToPositionWS(i, 2) * 4;
                                subChunksWithTriangles[subChunkIndex] = new CSubChunkWithTrianglesIndex(positionOfChunkWS + subChunkOffset, 0, true);

                                //todo re-add checking for this 
                                //   if (dynamicData.DistanceFieldChunkData.InstructionsChangedSinceLastFrame)
                                {
                                    TriangulationInstructions.AddNoResize(new CTriangulationInstruction(positionOfChunkWS + subChunkOffset, 0));
                            ...
                            {
                                subChunksWithTriangles[subChunkIndex] = default;
                            }
                        }
                    }
