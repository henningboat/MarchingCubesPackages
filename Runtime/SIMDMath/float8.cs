...
namespace SIMDMath
{
    ...
    public struct float8
    {
        ...
        public static float8 operator + (float8 lhs, float8 rhs) { return new float8 (lhs.a + rhs.a, lhs.a + rhs.b); }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float8 operator + (float8 lhs, float rhs) { return new float8 (lhs.a + rhs, lhs.a + rhs); }
        ...
        unsafe public float this[int index]
        {
            ...
            set
            {
                ...
                if ((uint)index >= 8)
                    throw new System.ArgumentException("index must be between[0...8]");
#endif
                fixed (float* array = &a.x) { array[index] = value; }
            }
        }
    }
